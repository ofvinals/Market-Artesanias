...
export default function Categorias() {
  ...
  return (
    <section className='w-full py-12 flex flex-col px-4 lg:px-[104px] '>
      <h1 className='text-tlv font-light text-general'>Categorías</h1>
      <div className='flex flex-wrap lg:flex-row justify-around lg:justify-between 2xl:justify-evenly mt-4'>
        {
          CATEGORIAS.map((categoria) => (
            <Link key={categoria.nombre} to='/categories' onClick={handleCategory(categoria.nombre)}>
              <div className='flex flex-col items-center relative group shadow-md rounded-[10px] mt-4 lg:mt-0' >
                <img src={categoria.imagen} alt={categoria.nombre} className='object-cover w-20 h-20 lg:h-[356px] lg:w-[228px] rounded-[10px]' />
                  <h2 className='absolute pt-10 text-specific lg:text-2xl  text-sm hover:opacity-100 '>{categoria.nombre}</h2>
                ...
                <div className='absolute inset-0 z-10 bg-black/40 hover:opacity-0 transition-opacity flex items-start justify-center rounded-[10px] pt-10'>
                  <h2 className='text-white lg:text-2xl  text-sm'>{categoria.nombre}</h2>
                </div>
              </div>
            </Link>
          ))
