...
const postAddProducto = async (req, res) => {
    const { Nombre, Disponible, Precio, Cantidad, Imagen, Descripcion, Genero, CategoryId } = req.body;
	...
	try {
        ...
        const response = await postAdd(Nombre, Disponible, Precio, Cantidad, Imagen, Descripcion, UserId, CategoryId, Genero);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({error: error.message});
    }
