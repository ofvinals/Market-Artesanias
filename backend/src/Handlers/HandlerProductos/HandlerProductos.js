...
const postAddProducto = async (req, res) => {
    ...
    try {
        const response = await postAdd(Email, Contraseña, Nombre, Apellido);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({error: error.mensage});
    }
...
const putUpdateProducto = async (req, res) => {
    const { Id, Email, Contraseña, Nombre, Apellido } = req.body;
    try {
        const response = await putUpdate(Id, Email, Contraseña, Nombre, Apellido);
